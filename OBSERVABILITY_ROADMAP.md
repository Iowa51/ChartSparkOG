...
- [ ] Admin analytics page (`src/app/(admin)/admin/analytics/page.tsx:121`) reads `submissions.user_id`, but the actual column is `provider_id` (verified 2026-04-20). Supabase returns the row without the missing column silently, so the page likely treats every row as having a null user_id — any per-clinician breakdown is broken. Fix to `provider_id`.
