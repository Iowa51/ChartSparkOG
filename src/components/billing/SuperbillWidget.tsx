...
export function SuperbillWidget() {
    const [lines, setLines] = useState<ServiceLine[]>([
        { id: '1', cptCode: '99214', modifiers: [], diagnosisPointers: [1], charge: 15000 },
    ]);
    ...
    return (
        <div className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden ring-1 ring-border/5">
            <div className="px-6 py-4 border-b border-border bg-slate-50 dark:bg-slate-900/50 flex items-center justify-between">
                ...
                <span className="text-[10px] font-black text-muted-foreground bg-muted p-1 rounded border border-border">EDI 837P READY</span>
            </div>
            ...
            <div className="p-6 space-y-6">
                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="border-b border-border">
                                <th className="pb-3 text-[10px] font-black uppercase tracking-widest text-muted-foreground w-24">CPT Code</th>
                                <th className="pb-3 text-[10px] font-black uppercase tracking-widest text-muted-foreground w-32">Modifiers</th>
                                <th className="pb-3 text-[10px] font-black uppercase tracking-widest text-muted-foreground w-20">Diag</th>
                                <th className="pb-3 text-[10px] font-black uppercase tracking-widest text-muted-foreground text-right w-24">Charge</th>
                                ...
                                <tr key={line.id} className="group transition-colors hover:bg-muted/5">
                                    ...
                                    <td className="py-4">
                                        <div className="flex gap-1">
                                            <span className="text-[10px] font-bold px-2 py-0.5 bg-primary/5 text-primary border border-primary/10 rounded cursor-pointer">+ Mod</span>
                                        </div>
                                    </td>
                                    <td className="py-4">
                                    ...
                                    <td className="py-4 text-right">
                                        <span className="text-sm font-bold text-foreground">${(line.charge / 100).toFixed(2)}</span>
                                    </td>
                                    <td className="py-4 text-right">
                                        <button
                ...
                <div className="pt-6 border-t border-border space-y-4">
                    ...
                    <button
                        ...
                        className={`w-full py-4 rounded-2xl flex items-center justify-center gap-2 text-sm font-black uppercase tracking-widest transition-all ${submitted
                ...
                <div className="p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl flex gap-3">
                    ...
                    <div>
                        <p className="text-xs font-bold text-amber-700 dark:text-amber-400">Final Compliance Check</p>
                        <p className="text-[10px] text-amber-600/80 dark:text-amber-500/80 mt-1 leading-relaxed">
                            By submitting, you attest that these services were medically necessary and documented in the clinical note. Claim will be scrubbed against NCCI edits before SFTP transmission.
                        </p>
                    </div>
                </div>
            </div>
