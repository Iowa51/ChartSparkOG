...
import { PilotReadonlyFormBanner } from "@/components/pilot/pilot-readonly-form-banner";
...
interface NewNoteFormProps {
  // Build 1.5: readonly courtesy UI for pilot organizations in their
  // post-active grace window. The API enforces 423 Locked via
  // assertMutationAllowed; this prop is defense-in-depth UI so the
...
export default function NewNoteForm({
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(({ data }: { data: { user: SupabaseUser | null } }) => {
      ...
      supabase
        ...
        .single()
        .then(
          ({ data }: { data: { first_name: string | null; last_name: string | null } | null }) => {
            ...
            if (!patientId && encounter.patient_id) {
              ...
              if (patientResponse.ok) {
                ...
                const formattedPatient = {
                  ...
                  status:
                    patientData.status === "active" ? "Active" : patientData.status || "Unknown",
                  avatarColor:
                    patientData.avatar_color ||
                    "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300",
                  lastVisit: "--",
  ...
  >("Subjective");
  const [userDotPhrases, setUserDotPhrases] = useState<
    { id: string; shortcut: string; expansion: string }[]
  >([]);
  ...
  useEffect(() => {
    ...
    if (savedDots) {
      try {
        setUserDotPhrases(JSON.parse(savedDots));
      } catch (e) {}
    }
  }, []);
  ...
  > = {
    ...
    "90833": {
      ...
      description:
        "Psychotherapy, 30 minutes with patient when performed with an evaluation and management service (add-on code).",
      details: [
    ...
    "90792": {
      ...
      description:
        "Psychiatric diagnostic evaluation with medical services, including history, mental status exam, and medical assessment.",
      details: [
    ...
    "F32.1": {
      ...
      description:
        "A mood disorder characterized by a single episode of depression with moderate severity. Patient experiences depressed mood and/or loss of interest lasting at least 2 weeks with functional impairment.",
      details: [
    ...
    "F41.1": {
      ...
      description:
        "A mental health disorder characterized by persistent and excessive worry about various aspects of life that is difficult to control, lasting at least 6 months.",
      details: [
    ...
    "F33.1": {
      ...
      description:
        "A mood disorder characterized by multiple episodes of major depression with moderate severity. Current or most recent episode meets criteria for moderate depressive episode.",
      details: [
    ...
    "G44.209": {
      ...
      description:
        "A primary headache disorder characterized by mild to moderate bilateral pressing or tightening pain that is not aggravated by routine physical activity.",
      details: [
    ...
    I10: {
      ...
      description:
        "High blood pressure without an identifiable secondary cause. Defined as systolic BP ≥130 mmHg and/or diastolic BP ≥80 mmHg on at least two occasions.",
      details: [
    ...
    "E11.9": {
      ...
      description:
        "A metabolic disorder characterized by high blood sugar, insulin resistance, and relative lack of insulin. This code indicates no documented micro- or macrovascular complications.",
      details: [
  ...
  const handleCodeClick = (code: string, type: "cpt" | "icd10") => {
    ...
    const explanation =
      codeExplanations[code] ||
      (() => {
        const libInfo = getCodeInfo(code);
        return libInfo
          ? { title: libInfo.title, description: libInfo.description, details: libInfo.details }
          : null;
      })();
    if (explanation) {
      setSelectedCodeInfo({
  ...
  const formatTriageSummary = (result: any): string => {
    ...
    if (result.drug_drug_interactions?.length) {
      ...
      result.drug_drug_interactions.forEach((ddi: any) => {
        lines.push(
          `  • ${ddi.med_a} + ${ddi.med_b} (${ddi.severity}) — ${ddi.clinical_significance || ddi.mechanism}`,
        );
  ...
  const fetchMedicationTriage = async (): Promise<string | null> => {
    ...
    try {
      ...
      const summary = formatTriageSummary({
        ...triageResult,
  ...
  const generateNoteFromPhrases = () => {
    ...
    const defaultPlanItems = [
      ...
      [
        ...
        "Encouraged daily physical activity",
      ...
      [
        ...
        "Supportive psychotherapy provided",
        "Stress reduction techniques reviewed",
        "Patient education provided",
        "Follow-up in 2 weeks",
      ],
      [
        "Current interventions effective; continue",
        "Behavioral activation strategies discussed",
        "Regular sleep schedule encouraged",
        "Warning signs reviewed",
        "Return visit in 3 weeks",
      ],
    ][variationSeed];
