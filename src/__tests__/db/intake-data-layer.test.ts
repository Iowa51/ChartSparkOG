...
const PHI_TABLES: { name: string; row: RowFactory }[] = [
    { name: 'intake_submissions', row: (org, patient) => ({ organization_id: org, patient_id: patient, status: 'patient_entered', responses: '{}' }) },
    { name: 'problems', row: (org, patient) => ({ organization_id: org, patient_id: patient, code_system: 'icd10', code: 'E11.9', display: 'Type 2 diabetes' }) },
    { name: 'medications', row: (org, patient) => ({ organization_id: org, patient_id: patient, name: 'Metformin', rxnorm_code: '860975' }) },
    { name: 'allergies', row: (org, patient) => ({ organization_id: org, patient_id: patient, allergen_type: 'drug', allergen_display: 'Penicillin', severity: 'moderate' }) },
    { name: 'family_history', row: (org, patient) => ({ organization_id: org, patient_id: patient, relative: 'mother', condition_display: 'Hypertension' }) },
    { name: 'social_history', row: (org, patient) => ({ organization_id: org, patient_id: patient, tobacco_status: 'never' }) },
    { name: 'immunizations', row: (org, patient) => ({ organization_id: org, patient_id: patient, vaccine_display: 'Influenza' }) },
    { name: 'ros_responses', row: (org, patient, sub) => ({ organization_id: org, patient_id: patient, intake_submission_id: sub, system: 'cardiovascular', finding: 'negative' }) },
];
...
const VITALS_TABLES: { name: string; row: RowFactory }[] = [
    { name: 'vitals', row: (org, patient) => ({ organization_id: org, patient_id: patient, recorded_by: CLINICIAN_ID, heart_rate: 72 }) },
    { name: 'screening_scores', row: (org, patient) => ({ organization_id: org, patient_id: patient, administered_by: CLINICIAN_ID, instrument: 'PHQ9', total_score: 5, item_responses: '{}' }) },
    { name: 'smart_triage_results', row: (org, patient) => ({ organization_id: org, patient_id: patient, triage_type: 'chart_summary', result_data: '{}' }) },
    { name: 'medication_interaction_log', row: (org, patient) => ({ organization_id: org, patient_id: patient, medication_a: 'DrugA', medication_b: 'DrugB', severity: 'low', action_taken: 'acknowledged', provider_id: CLINICIAN_ID }) },
];
...
async function cleanup(c: Client) {
    const childTables = [
        'ros_responses', 'problems', 'medications', 'allergies',
        'family_history', 'social_history', 'immunizations', 'intake_submissions',
        // Pre-existing vitals + siblings (org/patient-keyed, no cross-FK here).
        'vitals', 'screening_scores', 'smart_triage_results', 'medication_interaction_log',
    ];
    try {
        ...
        for (const t of childTables) {
            await c.query(
                ...
                [[ORG_A, ORG_B], [PATIENT_A, FOREIGN_PATIENT_ID]]
            ).catch(() => undefined);
        }
        // intake_templates has organization_id but no patient_id; keep system
        // (organization_id IS NULL) seed templates intact.
        await c.query(
            `DELETE FROM public.intake_templates WHERE organization_id = ANY($1::uuid[])`,
            [[ORG_A, ORG_B]]
        ).catch(() => undefined);
        await c.query(`DELETE FROM public.patients WHERE id = ANY($1::uuid[])`, [[PATIENT_A, FOREIGN_PATIENT_ID]]).catch(() => undefined);
        ...
        await c.query(`DELETE FROM public.organizations WHERE id = ANY($1::uuid[])`, [[ORG_A, ORG_B]]).catch(() => undefined);
    } finally {
...
beforeAll(async () => {
    ...
    await admin.query(`INSERT INTO public.organizations (id, name, slug) VALUES ($1,$2,$3), ($4,$5,$6)`, [
        ORG_A, `Org A ${SUFFIX}`, `org-a-${SUFFIX}`,
        ORG_B, `Org B ${SUFFIX}`, `org-b-${SUFFIX}`,
    ]);
    ...
    submissionA = await adminInsert('intake_submissions', { organization_id: ORG_A, patient_id: PATIENT_A, status: 'patient_entered', responses: '{}' });
    submissionB = await adminInsert('intake_submissions', { organization_id: ORG_B, patient_id: FOREIGN_PATIENT_ID, status: 'patient_entered', responses: '{}' });
});
...
describe('State machine: intake_submissions', () => {
    async function freshSubmission(): Promise<string> {
        return adminInsert('intake_submissions', {
            organization_id: ORG_A, patient_id: PATIENT_A, status: 'patient_entered', responses: '{}',
        });
    }
    async function setStatus(id: string, status: string) {
        await admin.query(`UPDATE public.intake_submissions SET status = $2 WHERE id = $1`, [id, status]);
    }
    ...
    test('legal path patient_entered -> provider_review -> reconciled -> signed', async () => {
        ...
        const { rows } = await admin.query<{ signed_snapshot: unknown }>(
            `SELECT signed_snapshot FROM public.intake_submissions WHERE id = $1`, [id]
        );
...
describe('Reconciled-row lock: children of a signed submission', () => {
    test('a reconciled problem cannot be mutated once its submission is signed; new versions still insert', async () => {
        ...
        await admin.query(`UPDATE public.intake_submissions SET status = 'provider_review' WHERE id = $1`, [subId]);
        await admin.query(`UPDATE public.intake_submissions SET status = 'reconciled' WHERE id = $1`, [subId]);

        const problemId = await adminInsert('problems', {
            organization_id: ORG_A, patient_id: PATIENT_A, intake_submission_id: subId,
            code_system: 'icd10', code: 'I10', display: 'Hypertension', reconciled: true,
        });

        await admin.query(`UPDATE public.intake_submissions SET status = 'signed' WHERE id = $1`, [subId]);
        ...
        const newVersion = await adminInsert('problems', {
            organization_id: ORG_A, patient_id: PATIENT_A, intake_submission_id: subId,
            code_system: 'icd10', code: 'I10', display: 'Hypertension (corrected)', reconciled: true,
        });
...
describe('State machine: INSERT is governed (SM-1)', () => {
        ...
        test(`INSERT with status='${badStatus}' is rejected`, async () => {
            await expect(
                adminInsert('intake_submissions', {
                    organization_id: ORG_A, patient_id: PATIENT_A, status: badStatus, responses: '{}',
                })
    ...
    test('INSERT carrying a pre-supplied signed_snapshot is rejected', async () => {
        await expect(
            adminInsert('intake_submissions', {
                organization_id: ORG_A, patient_id: PATIENT_A, status: 'patient_entered',
                responses: '{}', signed_snapshot: JSON.stringify({ forged: true }),
            })
    ...
    test('INSERT carrying a pre-supplied reviewed_by is rejected', async () => {
        await expect(
            adminInsert('intake_submissions', {
                organization_id: ORG_A, patient_id: PATIENT_A, status: 'patient_entered',
                responses: '{}', reviewed_by: CLINICIAN_ID,
            })
    ...
    test('INSERT at the initial state still succeeds (positive control)', async () => {
        const id = await adminInsert('intake_submissions', {
            organization_id: ORG_A, patient_id: PATIENT_A, responses: '{}',
        });
...
describe('State machine: signed_snapshot is server-derived (SM-2)', () => {
    ...
    async function reconciledSubmission(): Promise<string> {
        ...
        await admin.query(`UPDATE public.intake_submissions SET status='provider_review' WHERE id=$1`, [id]);
    ...
    test('a caller-supplied snapshot on sign is discarded; server rebuilds it from reconciled state', async () => {
        ...
        await adminInsert('problems', {
            organization_id: ORG_A, patient_id: PATIENT_A, intake_submission_id: id,
            code_system: 'icd10', code: 'I10', display: 'Hypertension', reconciled: true,
        });
...
describe('Vitals RLS remediation: getPatientLatestVitals read path (item 7.4)', () => {
    test('in-org clinician reads latest vitals for an in-org patient (feature keeps working)', async () => {
        await adminInsert('vitals', { organization_id: ORG_A, patient_id: PATIENT_A, recorded_by: CLINICIAN_ID, heart_rate: 80 });
    ...
    test('cross-org patient_id-only read now returns nothing (leak closed)', async () => {
        await adminInsert('vitals', { organization_id: ORG_B, patient_id: FOREIGN_PATIENT_ID, recorded_by: CLINICIAN_ID, heart_rate: 90 });
...
describe('anon role: deny-by-default (item 6)', () => {
    const ALL_TABLES = [
        'intake_templates', 'intake_submissions', 'problems', 'medications', 'allergies',
        'family_history', 'social_history', 'ros_responses', 'immunizations', 'vitals',
    ];
...
describe('Template catalog: intake_templates cross-org scoping (RLS-1 fix)', () => {
    ...
    test("clinician CANNOT read another org's ACTIVE template (leak closed)", async () => {
        const tplId = await adminInsert('intake_templates', {
            organization_id: ORG_B, specialty: `cardio_${SUFFIX}`, name: 'OrgB Active Tpl',
            version: 1, active: true, definition: '{"sections":[]}',
        });
        const n = await asUser(CLINICIAN_ID, async (c) => {
    ...
    test('clinician CAN read an own-org template even when inactive', async () => {
        const tplId = await adminInsert('intake_templates', {
            organization_id: ORG_A, specialty: `derm_${SUFFIX}`, name: 'OrgA Tpl',
            version: 1, active: false, definition: '{"sections":[]}',
        });
        const n = await asUser(CLINICIAN_ID, async (c) => {
            const { rows } = await c.query(`SELECT id FROM public.intake_templates WHERE id=$1`, [tplId]);
            return rows.length;
