...
export type AuditEventType =
  ...
  | "SCREENING_VIEW"
  | "SCREENING_CREATE"
  | "ASSESSMENT_ADMINISTER"
  | "ASSESSMENT_COMPLETE"
  | "ASSESSMENT_READ"
  | "ASSESSMENT_LIST"
  | "ASSESSMENT_TREND_READ"
  | "ASSESSMENT_SCALE_READ"
  | "ASSESSMENT_ASSIGNMENT_CREATE"
  | "ASSESSMENT_ASSIGNMENT_LIST"
  | "ASSESSMENT_ASSIGNMENT_DELETE"
  | "ENCOUNTER_VIEW"
  | "ENCOUNTER_CREATE"
  | "ENCOUNTER_UPDATE"
  | "ENCOUNTER_DELETE"
...
function sanitizeDetails(details: Record<string, any>): Record<string, any> {
  ...
  for (const [key, value] of Object.entries(details)) {
    ...
    if (phiFields.some((phi) => lowerKey.includes(phi))) {
      ...
      sensitiveDiagnosticFields.some((field) => lowerKey.includes(field)) &&
      typeof value === "string"
    ) {
...
export function getRiskLevel(eventType: AuditEventType): RiskLevel {
  const criticalEvents: AuditEventType[] = ["DATA_BREACH_SUSPECTED", "UNAUTHORIZED_ACCESS"];
...
const ALERT_DESCRIPTIONS: Record<string, string> = {
  DATA_BREACH_SUSPECTED:
    "A potential data breach has been detected. Immediate investigation is required.",
...
async function triggerSecurityAlert(entry: AuditLogEntry): Promise<void> {
  ...
  if (typeof window === "undefined") {
    try {
      ...
      const description =
        ALERT_DESCRIPTIONS[alertCode] || "A critical security event has been recorded.";
