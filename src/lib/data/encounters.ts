...
import { handleDatabaseError, safeLogger, validateRequired, createAuditLog } from "./utils";
 ...
 */
export async function completeEncounter(encounterId: string, userId: string): Promise<Encounter> {
  try {
    const supabase = await createClient();
 ...
 * Cancel an encounter
 */
export async function cancelEncounter(encounterId: string, userId: string): Promise<Encounter> {
  try {
    const supabase = await createClient();
