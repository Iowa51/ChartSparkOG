    ...
    try {
        ...
        if (supabase) {
            await supabase.from('smart_triage_results').insert({
                ...
                safety_score: result.overall_risk === 'green' ? 90 : result.overall_risk === 'yellow' ? 75 : result.overall_risk === 'red' ? 55 : 25,
                result_data: result,
                alerts_count: result.interactions.length,
                critical_alerts_count: result.interactions.filter(i => i.severity === 'critical' || i.severity === 'high').length,
