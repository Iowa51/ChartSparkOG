...
async function handlePost(context: RiskAssessmentContext): Promise<NextResponse> {
  ...
  if (user.role === "AUDITOR") {
    return NextResponse.json(
      { error: "Forbidden - Auditors cannot create assessments" },
      { status: 403 },
    );
  }
  ...
  try {
    ...
    logError({
      action: "RISK_ASSESSMENT_INVALID_JSON",
      error: sanitizeError(jsonError),
      userId: user.id,
    });
  ...
  if (assessmentData.mmse_score !== undefined)
    updateData.cognitive_score = assessmentData.mmse_score;
  if (assessmentData.moca_score !== undefined)
    updateData.cognitive_score = assessmentData.moca_score;
