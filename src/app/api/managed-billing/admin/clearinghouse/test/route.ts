...
// Allowed values come from the handler's own switch statement below
// (`claim_md`, `availity`, `office_ally`).
const ClearinghouseTestSchema = z
  .object({
    clearinghouse: z.enum(["claim_md", "availity", "office_ally"]),
  })
  .strict();
...
async function handlePost(context: AuthContext) {
  try {
    ...
    const validation = validateRequest(ClearinghouseTestSchema, body);
    ...
    const { clearinghouse } = validation.data;
      ...
      switch (clearinghouse) {
        case "claim_md":
          if (config.api_key_encrypted) {
            ...
            testResult = {
              success: response.ok,
              error: response.ok ? undefined : "API test failed",
            };
          } else {
        ...
        case "availity":
          if (config.api_key_encrypted && config.api_secret_encrypted) {
            ...
            testResult = {
              success: response.ok,
              error: response.ok ? undefined : "OAuth test failed",
            };
          } else {
    ...
    logAuditEventAsync({
      ...
      details: {
        action: "CLEARINGHOUSE_CONNECTION_TEST",
        clearinghouse,
