...
} from "@/lib/managed-billing/claim-validator";
...
async function handlePost(context: AuthContext) {
  try {
    const claimIdValidation = UUIDSchema.safeParse(context.params?.id);
    if (!claimIdValidation.success) {
      return NextResponse.json({ error: "Invalid claim id" }, { status: 400 });
    }
    const claimId = claimIdValidation.data;

    const result = await validateClaimForSubmission(
      claimId,
