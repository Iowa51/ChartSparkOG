...
import PrescribingCheckDialog from "@/components/smart-triage/PrescribingCheckDialog";
...
export default function EPrescribePage() {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        // Open the safety check dialog instead of sending directly
        if (medication.trim() && selectedPatient) {
            setShowSafetyCheck(true);
        } else {
            alert("Please select a patient and enter a medication.");
        }
    };
    ...
    return (
        <>
                            ...
                            <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
                                ...
                                <form className="p-8 space-y-6" onSubmit={handleSubmit}>
                                    ...
                                    >
                                        ...
                                        {prescriptionSent ? '✓ Prescription Transmitted' : 'Transmit Prescription'}
                                    </button>

                                    {prescriptionSent && (
                                        <div className="p-4 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-2xl text-sm text-emerald-700 dark:text-emerald-400 font-medium text-center animate-in fade-in slide-in-from-bottom-2 duration-300">
                                            ✓ Prescription sent successfully via Surescripts network.
                                        </div>
                                    )}
                                </form>
                            </div>
            ...
            {/* Prescribing Safety Check Dialog */}
            <PrescribingCheckDialog
                isOpen={showSafetyCheck}
                onClose={() => setShowSafetyCheck(false)}
                onProceed={() => handleSafetyAcknowledged()}
                medication={medication || 'New Medication'}
                dose=""
                patientId={selectedPatient || 'demo-patient'}
            />
        </>
    );
}
