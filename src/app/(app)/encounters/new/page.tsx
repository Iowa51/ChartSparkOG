    ...
    void (async () => {
      try {
        ...
        const scheduledEnd = new Date(
          scheduledStart.getTime() + selectedType.durationMinutes * 60 * 1000,
        );
        ...
        router.push(
          `/notes/new?template=${selectedType.template}&patientId=${selectedPatient.id}&encounterId=${encounterId}`,
        );
      } catch (err) {
      ...
      <main className="flex-1 p-6 lg:px-10 lg:py-8 max-w-4xl mx-auto w-full animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div className="flex items-center justify-between mb-12 px-6">
          <div className="flex items-center gap-4">
            <div
              className={`h-10 w-10 rounded-full flex items-center justify-center font-bold text-sm transition-all ${step >= 1 ? "bg-primary text-white shadow-lg shadow-primary/20" : "bg-muted text-muted-foreground"}`}
            >
              1
            </div>
          ...
          <div className="flex items-center gap-4">
            <div
              className={`h-10 w-10 rounded-full flex items-center justify-center font-bold text-sm transition-all ${step >= 2 ? "bg-primary text-white shadow-lg shadow-primary/20" : "bg-muted text-muted-foreground"}`}
            >
              2
            </div>
            <div className="hidden sm:block">
              ...
              {loading ? (
                ...
                filteredPatients.map((patient) => (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div
                        className={`h-12 w-12 rounded-full flex items-center justify-center font-bold text-lg ${getAvatarColor(patient)}`}
                      >
        ...
        {step === 2 && selectedPatient && (
          <div className="space-y-8">
            ...
            <div className="bg-primary/5 rounded-2xl p-6 border border-primary/20 flex items-center justify-between animate-in zoom-in-95 duration-300">
              <div className="flex items-center gap-4">
                <div
                  className={`h-12 w-12 rounded-full flex items-center justify-center font-bold text-lg ${getAvatarColor(selectedPatient)}`}
                >
              ...
              {visitTypes.map((type) => (
                ...
                >
                  <div className="flex items-start justify-between">
                    <div>
                      <p
                        className={`font-bold transition-colors ${selectedType.id === type.id ? "text-primary" : "text-foreground"}`}
                      >
                        {type.name}
                      </p>
